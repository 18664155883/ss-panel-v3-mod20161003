...
class UserController extends BaseController
{
    ...
    {			
        ...
        if ($traffic==511) {
			$res['msg'] = sprintf("恭喜你抽中  %d  我要要！奖励 6G 流量！", $traffic);
            $this->user->transfer_enable = $this->user->transfer_enable + Tools::toMB($traffic=6144);        
            $this->user->save();			
 		}	
        if ($traffic==520) {
			$res['msg'] = sprintf("恭喜你抽中  %d  我爱你！奖励 7G 流量！", $traffic);
        ...
        if ($traffic==555) {
			$res['msg'] = sprintf("恭喜你抽中  %d  中间数字！奖励 8G 流量！", $traffic);
            $this->user->transfer_enable = $this->user->transfer_enable + Tools::toMB($traffic=8192);          
            $this->user->save();
 		}			
        if ($traffic==566) {
			$res['msg'] = sprintf("恭喜你抽中  %d  我6 6！奖励 6G 流量！", $traffic);
            $this->user->transfer_enable = $this->user->transfer_enable + Tools::toMB($traffic=6144);          
            $this->user->save();
 		}			
        if ($traffic==588) {
        ...
        if ($traffic==599) {
			$res['msg'] = sprintf("恭喜你抽中  %d  我救救！奖励 9G 流量！", $traffic);
            $this->user->transfer_enable = $this->user->transfer_enable + Tools::toMB($traffic=9216);
            $this->user->save();
 		}			
        if ($traffic==600) {
			$res['msg'] = sprintf("恭喜你抽中  %d  这个末尾数字！奖励 10G 流量！", $traffic);
            $this->user->transfer_enable = $this->user->transfer_enable + Tools::toMB($traffic=10240);            
            $this->user->save();
 		}			
        if ($traffic==518) {
